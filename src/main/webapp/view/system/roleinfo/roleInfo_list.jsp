                    ...
                    if (result != null) {
                        ...
                        for (var i = 0; i < result.length; i++) {
                            ...
                            $("#info").append(
                                ...
                                "<th>"+ "<a href = 'roleInfo_show?roleId="+result[i].roleId+"&target=roleInfo_update'> 修改 </a>"+
                                "<a href = 'roleInfo_delete?roleId="+result[i].roleId+"' onclick = 'if(dele()==false)return false;'> 删除 </a>"+
                                "</th>"+
                                "</tr>")
                        }
                    }
...
<div class="row alert alert-info" style="margin:0px; padding:3px;">
    <form class="form-horizontal">
        ...
        <input type="button" class="btn btn-success" value="添加"
               onclick="window.location='view/system/roleinfo/roleInfo_add.jsp'"/>
    </form>
</div>
    ...
    <table id="info" class="table  table-condensed table-striped">
        ...
        <c:forEach items="${requestScope.roleList}" var="role">
            <tr>
                ...
                <td>${role.roleName}</td>
                <td>${role.roleDesc}</td>
                <th><a href="/roleInfo_show?roleId=${role.roleId}&target=roleinfo/roleInfo_update">修改</a>
                    <a href="/roleInfo_delete?roleId=${role.roleId}" onclick="if(dele()==false)return false;">删除</a>
                </th>
            </tr>
        </c:forEach>
